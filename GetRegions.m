...
InnerNormals[pts_?MatrixQ] :=
  Module[{np,dim,file,str,nf,fs,ns,mp=Total[pts]/Length[pts]},
{np,dim}=Dimensions@pts;
file = OpenWrite["./_qci.tmp"];
...
Close[file];
Run["qconvex Fv" <> " < " <> "./_qci.tmp" <> " > " <> "./_qco.tmp"];
file=OpenRead["./_qco.tmp"];
nf = Read[file,Number];
...
Options[GetRegions]={OutputForm->"Scalings","Rules"};
